...
export const StatsTable: React.FC<StatsTableProps> = ({ matches }) => {
  const statsData = matches.flatMap(match => {
    const players = [...match.homePlayers, ...match.awayPlayers];
    return players.map(player => ({
      player: `${player.lastName} ${player.firstName}`,
      team: player.team.name,
      role: player.role,
      total: player.servePoints + player.attackPoints + player.blockPoints,
      efficiency: player.playedSets ? (((player.servePoints + player.attackPoints + player.blockPoints) / player.playedSets) * 100).toFixed(1) : '-',
      ...
      points: player.servePoints + player.attackPoints + player.blockPoints,
      pointsPercentage: player.playedSets ? (((player.servePoints + player.attackPoints + player.blockPoints) / player.playedSets) * 100).toFixed(1) : '-'
    }));
  });
