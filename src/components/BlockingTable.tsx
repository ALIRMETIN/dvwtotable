...
export const BlockingTable: React.FC<BlockingTableProps> = ({ matches }) => {
  const blockingData = matches.flatMap(match => {
    const players = [...match.homePlayers, ...match.awayPlayers];
    return players.map(player => ({
      player: `${player.lastName} ${player.firstName}`,
      team: player.team.name,
      role: player.role,
      total: player.blockPoints,
      efficiency: player.playedSets ? ((player.blockPoints / player.playedSets) * 100).toFixed(1) : '-',
      ...
      pointsPercentage: player.playedSets ? ((player.blockPoints / player.playedSets) * 100).toFixed(1) : '-'
    }));
  });
