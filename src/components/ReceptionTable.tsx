...
export const ReceptionTable: React.FC<ReceptionTableProps> = ({ matches }) => {
  const receptionData = matches.flatMap(match => {
    const players = [...match.homePlayers, ...match.awayPlayers];
    return players.map(player => ({
      player: `${player.lastName} ${player.firstName}`,
      team: player.team.name,
      role: player.role,
      total: player.playedSets,
      efficiency: player.playedSets ? ((player.playedSets / player.playedSets) * 100).toFixed(1) : '-',
      ...
      points: player.playedSets,
      pointsPercentage: player.playedSets ? ((player.playedSets / player.playedSets) * 100).toFixed(1) : '-'
    }));
  });
